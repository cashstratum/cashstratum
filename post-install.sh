...
if command -v ufw &> /dev/null && [ -n "$API_ENV" ]; then
    ...
    echo "198.51.100.0/24). Leave BLANK for loopback-only, which is right if the"
...
# Administration helpers must reach this pool's /tmp socket from the host.
PrivateTmp=false
...
echo -e "${GREEN}✓ Systemd service created (cashstratum.service)${NC}"
...
#
# Runs as the actual user on purpose: cashstratum creates its logdir mode 0750, so
...
# ---------------------------------------------------------------------------
# CashStratum Notifier service (only if built)
# ---------------------------------------------------------------------------
NOTIFIER_BIN=""
if [ -x "$API_DIR/cashstratum-notifier" ]; then
    NOTIFIER_BIN="$API_DIR/cashstratum-notifier"
elif [ -x "$INSTALL_DIR/cashstratum-notifier" ]; then
    NOTIFIER_BIN="$INSTALL_DIR/cashstratum-notifier"
fi

NOTIFIER_ENV=""
if [ -f "$API_DIR/cashstratum-notifier.env" ]; then
    NOTIFIER_ENV="$API_DIR/cashstratum-notifier.env"
fi

NOTIFIER_SERVICE_FILE="/etc/systemd/system/cashstratum-notifier.service"
NOTIFIER_INSTALLED=false

if [ -n "$NOTIFIER_BIN" ] && [ -n "$NOTIFIER_ENV" ]; then
    NOTIFIER_INSTALLED=true
    chown root:root "$NOTIFIER_ENV" 2>/dev/null || true
    chmod 600 "$NOTIFIER_ENV" 2>/dev/null || true

    echo
    echo "Creating cashstratum-notifier systemd service..."
    cat > "$NOTIFIER_SERVICE_FILE" <<NOTIFYSVC
[Unit]
Description=CashStratum Notifier (ZMQ + log tailer)
Documentation=https://github.com/cashstratum/cashstratum/blob/main/api/NOTIFIER.md
After=network-online.target cashstratum.service
Wants=network-online.target
StartLimitIntervalSec=60
StartLimitBurst=3
...
Group=$ACTUAL_USER
WorkingDirectory=$API_DIR
ExecStart=$NOTIFIER_BIN
EnvironmentFile=$NOTIFIER_ENV
Restart=always
RestartSec=5
MemoryMax=64M
CPUQuota=10%
...
StandardOutput=journal
StandardError=journal
SyslogIdentifier=cashstratum-notifier
...
NOTIFYSVC
    ...
    echo -e "${GREEN}✓ cashstratum-notifier service created${NC}"
fi
...
if [ "$NOTIFIER_INSTALLED" = true ]; then
    echo
    echo -e "${YELLOW}Enable the CashStratum Notifier to start on boot?${NC}"
    read -p "(y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        systemctl enable cashstratum-notifier.service
        systemctl restart cashstratum-notifier.service 2>/dev/null || true
        sleep 1
        if systemctl is-active --quiet cashstratum-notifier.service 2>/dev/null; then
            echo -e "${GREEN}✓ cashstratum-notifier enabled and running${NC}"
        else
            echo -e "${RED}✗ cashstratum-notifier failed to start. Check: journalctl -u cashstratum-notifier -n 40${NC}"
        fi
    fi
fi
...
# Refresh the installed cleaner too; an older copy may not enforce the verified
# baseline required before the cron dry-run below.
if [ -f "$SCRIPT_DIR/clean-old-blocks.sh" ] && ! [ "$CLEANUP_SCRIPT" -ef "$SCRIPT_DIR/clean-old-blocks.sh" ]; then
...
if [ -f "$CLEANUP_SCRIPT" ]; then
    ...
    echo -e "${GREEN}✓ clean-old-blocks.sh installed${NC}"
    echo "Before scheduling, seed a verified historical index:"
    echo "  CASHSTRATUM_DIR=\"$INSTALL_DIR\" bash \"$CLEANUP_SCRIPT\" --seed-index /path/to/verified-heights.idx"
else
...
if [[ $REPLY =~ ^[Yy]$ ]] && [ -f "$CLEANUP_SCRIPT" ]; then
    # Verify using the same account, environment and index as the scheduled run.
    if runuser -u "$ACTUAL_USER" -- env CASHSTRATUM_DIR="$INSTALL_DIR" bash "$CLEANUP_SCRIPT" --dry-run; then
        # Cron uses /bin/sh and treats percent signs specially, even in quotes.
        CRON_DIR=$(printf '%s' "$INSTALL_DIR" | sed "s/'/'\\\\''/g; s/%/\\\\%/g")
        CRON_SCRIPT=$(printf '%s' "$CLEANUP_SCRIPT" | sed "s/'/'\\\\''/g; s/%/\\\\%/g")
        CRON_ID=$(printf '%s' "$INSTALL_DIR" | od -An -tx1 | tr -d ' \n')
        CRON_MARKER="# cashstratum-cleanup:$CRON_ID"
        CRON_JOB="0 3 * * * CASHSTRATUM_DIR='$CRON_DIR' bash '$CRON_SCRIPT' $CRON_MARKER"
        (crontab -u "$ACTUAL_USER" -l 2>/dev/null | grep -vFx "0 3 * * * $CLEANUP_SCRIPT" | grep -vF " $CRON_MARKER"; echo "$CRON_JOB") | crontab -u "$ACTUAL_USER" -
        echo -e "${GREEN}✓ Cron job added (runs clean-old-blocks.sh daily at 3 AM)${NC}"
    else
        echo -e "${YELLOW}Cleanup was not scheduled: initialize the verified index and resolve the dry-run failure first.${NC}"
    fi
fi
...
echo "  • /etc/systemd/system/cashstratum.service"
[ -n "$TESTNET_SERVICE_FILE" ] && echo "  • /etc/systemd/system/cashstratum-testnet.service"
[ "$API_INSTALLED" = true ] && echo "  • /etc/systemd/system/cashstratum-api.service"
[ "$NOTIFIER_INSTALLED" = true ] && echo "  • /etc/systemd/system/cashstratum-notifier.service"
echo
