...
# pr-gate.yml and the self-hosted build script all run without it -- so configure sets
...
# in this repo has ever parsed: release-gate.yml and the self-hosted build script both run
...
set -u
# Source tree to compile-check; defaults to the checkout this script lives in.
SRC_TREE="${SRC_TREE:-$(cd "$(dirname "$0")/.." && pwd)}"
SRC=/tmp/capnp-check-src
rm -rf "$SRC"; cp -a "$SRC_TREE" "$SRC"
