...
Because the pool runs `btcsolo` (`cashstratum -B`), all accounting is local and terminal: there is
...
3. Watch for a rise in `Above target` rejects. Miners that were mining below their assigned
   difficulty will now be rejected — which is correct, and is what standard CKPool/CashStratum behaviour dictates.
...
the live `cashstratum.conf` and restart. That removes the high-value Path A strings. It does
