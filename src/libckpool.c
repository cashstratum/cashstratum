...
static int __maybe_unused address_to_pubkeytxn(char *pkh, const char *addr)
{
	char b58bin[25] = {};
...
static int __maybe_unused address_to_scripttxn(char *psh, const char *addr)
{
	char b58bin[25] = {};
...
static int __maybe_unused segaddress_to_txn(char *p2h, const char *addr)
{
 ...
 *
 * The three *_to_txn() builders above are upstream's BTC path and are left
 * in place -- marked __maybe_unused -- rather than deleted, so future
 * merges with upstream keep applying cleanly. On BCH they are unreachable:
 * address_to_txn() below routes every address through the classifier, and
 * BCH has no segwit at all.
 * ------------------------------------------------------------------- */
...
static const char bch_b58_alphabet[] =
...
static bool bch_b58decode_strict(const char *s, uint8_t out25[25])
{
	...
	memset(out25, 0, 25);
	for (i = 0; i < len; i++) {
		const char *p = strchr(bch_b58_alphabet, s[i]);
		int carry, j;

		if (!p || s[i] == '\0')
			return false;
		carry = (int)(p - bch_b58_alphabet);
		for (j = 24; j >= 0; j--) {
			carry += 58 * out25[j];
			out25[j] = carry & 0xff;
			carry >>= 8;
