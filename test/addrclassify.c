 ...
 * validate/construct path the pool uses. Upstream ckpool 1.2.0 replaced
 * jansson with yyjson, which is compiled into the daemon rather than into
 * libckpool.a, so the weak jansson stubs this test used to carry are gone.
 *
 ...
 * cashaddr/legacy address pairs already proven correct by the existing
 * cashaddr suite (test/cashaddr.c), cross-checked against that same local decoder.
...
/* Known-good CashAddr/legacy pairs, already proven correct by the existing
 * cashaddr suite (test/cashaddr.c) -- all mainnet P2PKH. */
typedef struct {
