...
# The poll loop breaks the moment the height increases, so a
...
stratum_probe() {
	...
	# result fails on a perfectly healthy pool. Observed on a CI run
	# where scenario 5b failed this way while scenario 5 --
...
scenario_8_per_user_round_independence() {
	...
	#      one CI run: 7.415 vs winning 1.072 (above)
	#      another:    2.735 vs winning 10.472 (below)
	#
	...
	#      observed on CI: best=4.9005 ever=4.
	#
...
# landed in the fix for this defect) now lowercases a cashaddr
