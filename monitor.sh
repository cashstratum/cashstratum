...
# CashStratum / CKPool Monitor - Matrix Style
# Colors
GREEN='\033[0;32m'
...
CASHSTRATUM_DIR="${CASHSTRATUM_DIR:-$HOME/cashstratum}"
CKPOOL_DIR="${CKPOOL_DIR:-$HOME/ckpool}"

if [ -f "$CASHSTRATUM_DIR/logs/cashstratum.log" ]; then
    LOG_FILE="$CASHSTRATUM_DIR/logs/cashstratum.log"
elif [ -f "$CKPOOL_DIR/logs/ckpool.log" ]; then
...
if [ ! -f "$LOG_FILE" ]; then
    ...
    echo "Set CASHSTRATUM_DIR or CKPOOL_DIR environment variable, or ensure logs exist"
    exit 1
fi
...
echo -e "${BRIGHT_GREEN}                  CASHSTRATUM / CKPOOL MONITOR SYSTEM                  ${NC}"
echo -e "${BRIGHT_GREEN}▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓${NC}"
