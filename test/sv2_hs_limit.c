...
int main(void)
{
	...
	for (i = 0; i < 30; i++) {
		if (!sv2_handshake_try_reserve("192.0.2.1")) {
			fprintf(stderr, "FAIL: reserve %d/30 for 192.0.2.1\n", i + 1);
			fails++;
			break;
		}
	}
	expect(i == 30, "30 reserves for one IP succeed");
	expect(!sv2_handshake_try_reserve("192.0.2.1"),
	       "31st reserve for same IP is denied");

	/* Different IP still allowed (global not full at 30). */
	expect(sv2_handshake_try_reserve("192.0.2.2"),
