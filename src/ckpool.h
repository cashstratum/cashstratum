...
/* One entry of the config's "mindiff_overrides" object: a case-insensitive
 * useragent substring and the difficulty floor to apply to clients matching
 * it. See ckpool_instance.mindiff_overrides. */
...
struct ckpool_instance {
	...
	char **btcdpass;
	bool *btcdnotify;
	char **btcdzmq;      // ZMQ endpoints for each btcd server ("zmqnotify" key)
	...
	/* Difficulty floors for rental services, keyed by a substring of the
	 * miner's useragent, parsed out of the config's "mindiff_overrides"
	 * object. Copied into owned storage at parse time: upstream frees the
	 * config yyjson_doc when parse_config() returns and, unlike jansson,
	 * yyjson has no refcount with which to hold a value alive past it. */
	mindiff_override_t *mindiff_overrides;
	int mindiff_overrides_count;
	...
	/* Upstream's developer-donation coinbase output is removed here: its
	 * three hard-coded addresses are bech32 BTC addresses that the BCH
	 * classifier cannot encode, so a donation output would have carried a
	 * zero-length payout script. The pool operator fee above replaces it. */
...
/* Upstream leaves these three as placeholders, so every command answering
 * through them -- users, workers, clients, getuser, getworker, userclients,
 * workerclients and the generator's proxy commands -- accepts the connection
 * and replies with nothing at all. Only `stats` and `ping` appear to work,
 * because they call send_unix_msg() directly. A caller that gets no reply
 * blocks until its own read times out.
 *
 * All three consume/own what they are given, matching how every call site
 * already behaves. */
static inline yyjson_mut_doc *yyjson_errormsg(const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	if (unlikely(vasprintf(&buf, fmt, ap) < 0))
		buf = NULL;
	va_end(ap);
...
static inline yyjson_mut_doc *yyjson_encode_errormsg(yyjson_read_err *err_val)
{
	if (!err_val)
		return yyjson_errormsg("json decode error");
	return yyjson_errormsg("json decode error at position %zu: %s", err_val->pos,
			       err_val->msg ? err_val->msg : "unknown");
}

/* A NULL doc is a caller error path; answer "{}" so the client gets a
 * parseable reply instead of blocking until its read times out. */
static inline void send_api_yyresponse(yyjson_mut_doc *doc, const int sockd)
{
	char *buf;
