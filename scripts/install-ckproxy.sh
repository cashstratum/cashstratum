...
# Cloned from CashStratum repository per Decision D5.
...
detect_distro() {
    ...
    case " $DISTRO $DISTRO_LIKE " in
        *" ubuntu "*|*" debian "*)
            ...
            # libsodium-dev is required by Stratum V2; without it configure
            # silently resolves enable_sv2=auto to "no".
            PACKAGES="build-essential git autoconf automake libtool pkg-config yasm libzmq3-dev libsodium-dev"
            ;;
        *" fedora "*|*" rhel "*|*" centos "*)
            PKG_MANAGER="dnf"
            INSTALL_CMD="dnf install -y"
            UPDATE_CMD="dnf check-update || true"
            PACKAGES="gcc gcc-c++ make git autoconf automake libtool pkgconf-pkg-config yasm zeromq-devel libsodium-devel"
            ;;
        *)
...
# Fetch latest CashStratum source
if [ -d "$SRC_DIR/.git" ]; then
    echo "Updating existing CashStratum source in $SRC_DIR..."
    git -C "$SRC_DIR" pull
else
    echo "Cloning CashStratum into $SRC_DIR..."
    git clone "$GIT_URL" "$SRC_DIR"
fi
[ "$SRC_DIR" != "/opt/ckproxy" ] && ln -sfn "$SRC_DIR" /opt/ckproxy 2>/dev/null || true

# Build and install
cd "$SRC_DIR"
./autogen.sh
# --enable-sv2, not the default "auto". This builds UPSTREAM ckpool (see
# GIT_URL), so the "Stratum V2 is Bitcoin-only" reasoning that makes
# repo-root install-ckpool.sh, build-on-beast.sh and release-gate.yml pass
# --disable-sv2 does not apply here -- those build this BCH fork; this builds
# a Bitcoin proxy, where SV2 is wanted. The prompt below offers an SV2
# upstream URL and generator.c's parser for it is #ifdef HAVE_SV2, so an
# auto-detected SV2=no ships a proxy that cannot use the option it just
# advertised. --enable-sv2 makes configure stop and name what is missing
# instead. Matches install-ckpool-solo.sh, which builds the same upstream.
