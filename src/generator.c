...
static void detect_cashaddr_prefix(connsock_t *cs)
{
	 ...
	 * getblockchaininfo:
	 *
	 *     -testnet  -> "test"       -testnet4 -> "test4"
	 *     -scalenet -> "scale"      -chipnet  -> "chip"
	 *     -regtest  -> "regtest"    (mainnet) -> "main"
	 *
	 * Only "test" and "regtest" happen to match their flag. The long
	 * spellings are kept below for other node implementations that may emit
	 * them, but they are dead code against BCHN -- do NOT delete the short
	 * forms thinking they are the redundant ones.
	 *
	 * Getting this wrong is silent and expensive: the chain falls through to
	 * the default bitcoincash prefix, every bchtest: address is rejected as
	 * invalid, no coinbase is ever built, and the pool reports the thoroughly
	 * misleading "CRITICAL: No bitcoinds active!" on a perfectly healthy
	 * node. Always confirm a new chain token against a live node rather than
...
static bool server_alive(server_instance_t *si, bool pinging)
{
	...
	gbtbase_t gbt;
	int fd;
	/* segwit is always false on BCH; ckpool no longer stores it, so a
	 * throwaway local receives validate_address()'s output param. */
	bool segwit_unused;
	 ...
	 * rtdonaddress are bech32 BTC addresses that BCH cannot even encode. */

	if (!ckpool.node && !validate_address(cs, ckpool.btcaddress, &ckpool.script, &segwit_unused)) {
		LOGWARNING("Invalid bchaddress: %s !", ckpool.btcaddress);
		goto out;
	}
...
/* Defined further down; needed here by the failover paths below. */
...
bool generator_get_blockhash(int height, char *hash)
{
	...
	if (unlikely(!ret)) {
		...
		/* Ask the GENERATOR thread to fail over; never call
		 * live_server() from here.
		 *
		 * generator_get_blockhash(), generator_getbase() and
		 * generator_getbest() all run on the stratifier's threads --
		 * this one from local_block_submit(), i.e. the block
		 * submission path itself. live_server() belongs to the
		 * generator thread: it assigns gdata->current_si, and
		 * server_alive() can dealloc(cs->auth) on a connsock the
		 * generator thread may be mid-request on. Worse, when no
		 * bitcoind answers it loops on `sleep(5)` without bound, so
		 * calling it here BLOCKS the stratifier. That is visible in
		 * production on 2026-08-28 08:53:53 as a ~95 s work-update
		 * stall, "CRITICAL: No bitcoinds active!" interleaved with
		 * "Generator returned failure in update_base".
		 *
		 * The inline call bought nothing anyway. gen_loop() checks
		 * !si->alive on its next message and jumps to its own
		 * live_server() -- the same function, reached the same way --
		 * so the failover behaviour is identical; only the thread it
		 * runs on differs. This is also exactly what upstream does. */
		reconnect_generator();
	}
...
struct genwork *generator_getbase(void)
{
	...
	if (unlikely(!gen_gbtbase(cs, gbt))) {
		...
		/* Async only -- see generator_get_blockhash() above for why
		 * live_server() must not run on the stratifier thread. The
		 * comment this replaces claimed an async reconnect "would keep
		 * retrying the failed server". It does not, and it is not
		 * merely better -- gen_loop() reaches the SAME live_server()
		 * with the same argument, so the server selection is
		 * byte-for-byte the behaviour the inline call had. The only
		 * thing that changes is which thread pays for it. */
		reconnect_generator();
...
int generator_getbest(char *hash)
{
	...
	if (unlikely(!get_bestblockhash(cs, hash))) {
		...
		/* Async only -- see generator_get_blockhash() above. */
		reconnect_generator();
		goto out;
	}
	ret = GETBEST_SUCCESS;
...
bool generator_checkaddr(const char *addr, bool *script, bool *segwit)
{
	...
	/* DEBUG, not WARNING: same unauthenticated-input reasoning as
	 * validate_address() in bitcoin.c, which has already logged this
	 * failure one frame down. */
	if (unlikely(!ret))
		LOGDEBUG("Address validation failed for %s via %s:%s", addr, cs->url, cs->port);
out:
	return ret;
}
