...
/* submission_diff() returns this instead of a share difficulty when the
 * client has no per-user coinbase for the workbase it submitted against.
 * Negative so it can never be confused with a real difficulty, which
 * diff_from_target() always returns as a positive value. */
#define SUBMISSION_NO_COINBASE (-1.0)
...
struct user_instance {
	 ...
	 * address: authorised anyway (smart-fallback decision D2), mining to
	 * the pool's own bchaddress via a copy of sdata->txnbin/txnlen. */
	bool pool_fallback;
	...
	int64_t shares;
	int64_t round_shares; /* Accumulated diff this round; zeroed for the finder on a block solve
				 (or every user on an operator reset), drives per-user luck */
...
static void generate_coinbase(workbase_t *wb)
{
	...
	/* Upstream hard-codes a leading "ckpool" push here. The fork drops it so
	 * the coinbase message is entirely operator-configurable via btcsig --
	 * the on-chain scriptSig of all 81 mainnet blocks this pool has solved
	 * carries btcsig and nothing else. */
	wb->coinb2len = 0;
	if (ckpool.btcsig) {
	...
	/* Upstream sets nSequence to 0xfffffffe for BIP54, which is a Bitcoin
	 * proposal with no Bitcoin Cash equivalent. Keep the final-input
	 * 0xffffffff this pool has mined all 81 of its mainnet blocks with:
	 * a non-final nSequence makes nLockTime enforceable, which buys nothing
	 * on BCH and changes consensus-relevant coinbase bytes for no benefit. */
	memcpy(wb->coinb2bin + wb->coinb2len, "\xff\xff\xff\xff", 4);
	wb->coinb2len += 4;

	// Generation value
	g64 = wb->coinbasevalue;
	/* Pool operator fee: a second coinbase output paying ckpool.poolfee
	 * percent of the generation value to ckpool.pooladdress. This is the
	 * reason the fork exists; it replaces upstream's developer donation
	 * output entirely. */
	if (ckpool.poolvalid && ckpool.poolfee > 0) {
		double dbl64 = (double)g64 / 100 * ckpool.poolfee;
	...
	if (ckpool.poolvalid && ckpool.poolfee > 0 && !poolfee_dust) {
		u64 = htole64(d64);
	...
	/* Blank lock time. Upstream sets nLockTime to height-1 for BIP54, a
	 * Bitcoin proposal BCH has not adopted; combined with the 0xffffffff
	 * nSequence restored above, a zero nLockTime is what every block this
	 * pool has solved carries. */
	wb->coinb3len += 4;
	...
	if (!ckpool.btcsolo) {
		...
		if (unlikely(!ckpool.coinbase_valid)) {
			...
			if (ckpool.poolfee > 0 && ckpool.poolvalid)
				LOGWARNING("%.1f percent pool operator fee to %s", ckpool.poolfee, ckpool.pooladdress);
		}
...
static void __generate_userwb(sdata_t *sdata, workbase_t *wb, user_instance_t *user)
{
	...
	/* A user with no payout script has nothing to splice in. Writing the
	 * length byte anyway emits a zero-length output script, i.e. coins
	 * paid to nothing. generate_userwbs() already skips these, but the
	 * parse_authorise() and renotify_solo_client() call sites reach here
	 * with a single user and no such check, so guard the shared path. */
	if (unlikely(!user->txnlen))
		return;
...
/* Unreachable on BCH (no default_witness_commitment is ever returned); kept
...
retry:
		...
		if (unlikely(!wb)) {
			if (retries++ < 5 || *prio == GEN_PRIORITY) {
				...
				/* Brief delay to give the generator thread a
				 * chance to act on the reconnect request
				 * generator_getbase() just sent it; without it
				 * all five retries are spent inside the same
				 * failed window. The failover is asynchronous
				 * -- these retries do not wait for it, they
				 * only avoid busy-spinning while it happens. */
				cksleep_ms(100);
				goto retry;
			}
		 ...
		 * segwit); if a node ever sends one, warn and ignore it rather
		 * than building BTC-style witness commitment data. Every
		 * insert_witness code path below therefore stays dormant. */
...
/* Zero just the pool-wide best diff/share stats. Shared by both
 * reset_bestshares() and reset_bestshares_unattributed() below. */
static void reset_pool_bestshares(sdata_t *sdata)
{
...
/* Reset the pool-wide best diff/share stats on every block solve, and the
 * per-user round_shares/best_diff for just the finder if one is given, or
 * for every user if finder is NULL. The NULL-finder, reset-everyone path is
 * for the operator's resetshares command only: an unattributable solve
 * (finder could not be resolved) must NOT wipe every user's round -- that
 * is exactly the behaviour the per-user round existed to remove, so those
 * call sites use reset_bestshares_unattributed() instead. */
static void reset_bestshares(sdata_t *sdata, user_instance_t *finder)
{
	user_instance_t *user, *tmpuser;
	stratum_instance_t *client, *tmp;

	reset_pool_bestshares(sdata);

	ck_rlock(&sdata->instance_lock);
	HASH_ITER(hh, sdata->stratum_instances, client, tmp) {
		if (finder && client->user_instance != finder)
			continue;
		client->best_diff = 0;
	}
...
/* A block solve whose finder could not be resolved (no workername on the
 * local solve message, or a remote block naming a user we don't know).
 * Reset only the pool-wide counters -- every user's round_shares/best_diff
 * is left intact, since wiping them for a solve nobody can be credited with
 * is exactly the pool-wide-reset behaviour the per-user round existed to
 * remove. */
static void reset_bestshares_unattributed(sdata_t *sdata)
{
	reset_pool_bestshares(sdata);
	LOGWARNING("Block solve could not be attributed to a user, resetting pool-wide bestshare only");
}

/* Buffer size for canonicalise_username()'s output: the longest cashaddr
 * prefix ("bitcoincash", 11 chars) + ':' + a 127-char username + NUL. */
#define CANON_USERNAME_SIZE (11 + 1 + 127 + 1)

/* Canonicalise a username to one user key so that bare, prefixed, and
 * mixed-case spellings of the same CashAddr all collapse onto the same
 * account: <prefix>:<lowercase payload>, where <prefix> is the chain's own
 * bch_get_cashaddr_prefix() (bitcoincash / bchtest / bchreg), detected at
 * startup. Legacy Base58 (no ':' and length != 42) is copied byte-for-byte
 * unchanged -- it is case-significant and prefix-less. Non-address
 * usernames are copied unchanged too. `in` and `out` must not overlap.
 * When !ckpool.proxy this makes one local, no-RPC generator_checkaddr()
 * call. */
static void canonicalise_username(const char *in, char *out, size_t outlen)
{
	bool addr_valid = false, script_unused = false, segwit_unused = false;

	if (!ckpool.proxy)
		addr_valid = generator_checkaddr(in, &script_unused, &segwit_unused);

	if (addr_valid && (strchr(in, ':') || strlen(in) == 42)) {
		char *p;

		if (strchr(in, ':'))
			snprintf(out, outlen, "%s", in);
		else
			snprintf(out, outlen, "%s:%s", bch_get_cashaddr_prefix(), in);
		for (p = out; *p; p++)
...
static user_instance_t *user_by_workername(sdata_t *sdata, const char *workername)
{
	...
	char canon_username[CANON_USERNAME_SIZE];
	...
	/* Find the user first */
	canonicalise_username(username, canon_username, sizeof(canon_username));
	user = get_user(sdata, canon_username);
	return user;
}
...
static void block_solve(yyjson_mut_doc *doc)
{
	...
	/* finder is NULL here only when the block message carried no
	 * workername (the !workername branch above) -- user_by_workername()
	 * always creates via get_user(), so it can never itself return NULL
	 * and leave finder dangling. */
	if (finder)
		reset_bestshares(sdata, finder);
	else
		reset_bestshares_unattributed(sdata);
}
 ...
 * count. */
/* Look up a mindiff_overrides entry by exact (case-insensitive) key, e.g.
...
static int64_t mindiff_override_diff(const char *key, const int64_t fallback)
{
	...
	for (i = 0; i < ckpool.mindiff_overrides_count; i++) {
				       ...
				       yyjson_mut_val *params_val)
{
	...
	/* Rental services in the useragent get a difficulty floor applied
	 * immediately, before authorise.
	 *
	 * NiceHash sends a useragent like "NiceHashMiner/1.0.0".
	 * MiningRigRentals sends "MiningRigRentals" ONLY on its pool-test
	 * connection; live rentals present as "MRR-Hash/<ver> <seller> ...",
	 * so both spellings must be matched or a real MRR rental never gets
	 * the floor and the marketplace flags the pool. */
	if (strcasestr(client->useragent, "nicehash")) {
		int64_t nicehash_diff = mindiff_override_diff("nicehash", 500000);

		client->suggest_diff = nicehash_diff;
		client->diff = client->old_diff = nicehash_diff;
		client->rental_diff = true;
		LOGNOTICE("NiceHash detected from useragent '%s' for client %s, applied difficulty %"PRId64,
			  client->useragent, client->identity, nicehash_diff);
	} else if (strcasestr(client->useragent, "miningrigrentals") ||
		   strcasestr(client->useragent, "mrr-hash")) {
		int64_t mrr_diff = mindiff_override_diff("miningrigrentals", 1000000);

		client->suggest_diff = mrr_diff;
		client->diff = client->old_diff = mrr_diff;
		client->rental_diff = true;
		LOGNOTICE("MiningRigRentals detected from useragent '%s' for client %s, applied difficulty %"PRId64,
...
static void read_userstats(sdata_t *sdata, int tvsec_diff)
{
	...
	while ((dir = readdir(d)) != NULL) {
		...
		if (!strcmp(username, "/") || !strcmp(username, ".") || !strcmp(username, ".."))
			continue;
		/* A real username can never contain '.' or '_' since authorise()
		 * splits the workername on the first of either character to derive
		 * it. Any file whose name contains one of these is a backup or
		 * stray file (e.g. a .bak dropped in the users directory), not a
		 * genuine user file, so skip it to avoid creating a phantom user. */
		if (strpbrk(username, "._")) {
			LOGNOTICE("Ignoring non-user file %s in users directory", username);
			continue;
		}
		...
		yyjson_obj_get_int64(&user->round_shares, val, "roundshares");
...
/* Find user by username, returning NULL if it doesn't exist. Never creates
 * a user, so it's safe to call on untrusted input (e.g. remote messages)
 * where get_create_user() would be unwanted. */
static user_instance_t *find_user(sdata_t *sdata, const char *username)
{
	user_instance_t *user;
 ...
 * names such as "rig01", "alice" or "worker". Used by the solo auth
				      ...
				      const char *workername)
{
	 ...
	 * lookup -- this is fully local (no RPC round trip), so doing it here
	 * is cheap. Kept separate from canonicalise_username() below because
	 * address_to_txn() further down needs addr_script and addr_segwit,
	 * not just a validity bool. */
	if (!ckpool.proxy)
		addr_valid = generator_checkaddr(username, &addr_script, &addr_segwit);

	/* Canonicalise to <prefix>:<lowercase payload> so "BITCOINCASH:QQ...",
	 * an uppercase bare cashaddr, and the canonical lowercase form all
	 * resolve to the same user account instead of silently creating
	 * duplicates. Legacy Base58Check addresses are case-significant and
	 * prefix-less, and are left untouched by canonicalise_username(). The
	 * canonical form needs its own storage: `username` still points into
	 * base_username, which has no room for a prepended prefix. */
	canonicalise_username(username, canon_username, sizeof(canon_username));
	...
	if (!ckpool.proxy && (new_user || !user->btcaddress)) {
		 ...
		 * mines to the pool's own bchaddress. */
		if (!addr_valid && ckpool.btcsolo) {
			...
			const bool ret)
{
	if (ret) {
		...
		if (ckpool.proxy) {
			...
			LOGNOTICE("Authorised client %s %s worker %s as user %s with diff %"PRId64,
				  client->identity, client->address, client->workername,
				 ...
				 user_instance_t *user_instance)
{
	...
	/* __user_notify() returns NULL when the user has no userwb for this
	 * workbase. Sending that on would hand stratum_add_yysend() a NULL
	 * document; drop it and say so instead. */
	if (unlikely(!doc)) {
		LOGWARNING("No userwb to notify client %"PRId64" user %s of workbase %"PRId64,
			   client_id, user_instance->username, wb->id);
		return;
	}
	stratum_add_yysend(sdata, doc, client_id, SM_UPDATE);
}

/* A solo client submitted a share against a workbase it has no per-user
 * coinbase for. The share is rejected by the caller; give the client work it
 * can actually mine by generating its userwb for the CURRENT workbase and
 * notifying it. The submitted workbase is deliberately not repaired: a
 * coinbase minted now is not the one that mining.notify described, so a share
 * against it would be just as unvalidatable.
 *
 * Throttled to once a second across all clients. This path has never been
 * observed in production (0 occurrences in 77,067 log lines), but a client
 * that did hit it would hit it on every submit, and neither a log line nor a
 * notify per share is something to hand an unbounded share rate.
 *
 * Needs to be entered with the client holding a ref count and no locks held. */
static void renotify_solo_client(sdata_t *sdata, const stratum_instance_t *client)
{
	static time_t last_renotify;
	time_t now_t = time(NULL), prev;
	workbase_t *wb;

	if (!ckpool.btcsolo || !client->user_instance)
		return;
	/* SV2 sessions are not connector clients; work reaches them through
	 * sv2_strat_on_work_update. An SV1 mining.notify would fail to send
	 * and drop the virtual instance -- the same reason stratum_broadcast()
	 * skips them. */
	if (client->sv2)
		return;
	/* create_ckmsgqs("sprocessor", ...) runs nproc/2 threads draining one
	 * queue, so several can be inside this function at once. A plain
	 * read-compare-write on last_renotify lets all of them see the old
	 * value and every one proceed, which is exactly the unbounded notify
	 * rate the throttle exists to prevent. Exchange makes the winner
	 * unique. */
	prev = __atomic_exchange_n(&last_renotify, now_t, __ATOMIC_SEQ_CST);
	if (now_t == prev)
		return;
	...
	update_solo_client(sdata, wb, client->id, client->user_instance);
			    ...
			    yyjson_mut_doc **err_doc)
{
	...
	/* Reject control characters outright. The workername is stored, is
	 * echoed into the sharelog record, and reaches half a dozen log
	 * format strings; a newline in it lets an unauthenticated client
	 * forge whole log lines, up to and including a fake block-solve
	 * record. No legitimate miner sends one -- a stratum username is a
	 * JSON string, so a real control character has to be deliberately
	 * escaped to get here. DEL (0x7f) is included; bytes >= 0x80 are not,
	 * as they cannot terminate or start a log line and rejecting them
	 * would break non-ASCII worker names that work today. */
	{
		const unsigned char *p;

		for (p = (const unsigned char *)buf; *p; p++) {
			if (unlikely(*p < 0x20 || *p == 0x7f)) {
				*err_doc = yyjson_string("Control character in username");
				goto out;
			}
		}
	}
	...
	/* Worker-name pattern difficulty overrides.
	 *
	 * Skipped entirely for rental clients already handled by the useragent
	 * detection in parse_subscribe(): with BCH-address usernames a short
	 * override key (e.g. "s9") can substring-match inside the address and
	 * clobber the rental service's already-applied difficulty, causing
	 * NiceHash to reject the rig. */
	...
	} else if (ckpool.mindiff_overrides_count) {
		/* Match only the worker-name segment, never the address. A BCH
		 * cashaddr shares its base32 charset with these short keys and
		 * can substring-collide inside the address (see the "s9" case
		 * noted above), and matching the whole string would also let any
		 * miner select a high-difficulty override simply by naming its
		 * worker after one. */
		const char *wname = strpbrk(client->workername, "._");
		int i;

		wname = wname ? wname + 1 : NULL;
		for (i = 0; wname && i < ckpool.mindiff_overrides_count; i++) {
			const char *pattern = ckpool.mindiff_overrides[i].pattern;
			int64_t override_diff = ckpool.mindiff_overrides[i].diff;

			/* Case-insensitive substring match for worker patterns */
			if (!strcasestr(wname, pattern) || override_diff <= 0)
				continue;
			/* Clamp to the pool's configured bounds -- an override
			 * must never pin a client outside [mindiff, maxdiff]. */
			if (ckpool.mindiff && override_diff < ckpool.mindiff)
				override_diff = ckpool.mindiff;
			if (ckpool.maxdiff && override_diff > ckpool.maxdiff)
				override_diff = ckpool.maxdiff;
			client->suggest_diff = override_diff;
			/* Apply immediately for NiceHash compatibility */
			client->diff = client->old_diff = override_diff;
			LOGNOTICE("Client %s workername '%s' matched pattern '%s', applied difficulty %"PRId64,
	...
	/* Ensure the client has a valid difficulty set - use startdiff if
	 * nothing above assigned one. */
	if (!client->diff) {
		client->diff = client->old_diff = ckpool.startdiff;
		LOGINFO("Client %s using default startdiff %"PRId64, client->identity, client->diff);
	}
	...
	if (!ckpool.btcsolo || user->btcaddress || user->pool_fallback)
		ret = true;
	else if (looks_like_address(user->username)) {
		*err_doc = yyjson_string("Invalid BCH address (bad checksum or wrong network) - "
...
out:
	if (ckpool.btcsolo && ret && !client->remote) {
		...
		/* No stratum_send_diff() here. sauth_process() reconciles
		 * client->diff against mindiff/suggest_diff after this returns and
		 * sends the difficulty itself, immediately after the authorisation
		 * response. Sending here would announce the pre-reconciliation
		 * value a moment before that, so a solo client saw THREE
		 * mining.set_difficulty in the subscribe -> authorize window
		 * (subscribe-time floor, this one, then the reconciled value).
		 * Dropping it leaves the last-announced difficulty byte-identical
		 * and matches what a non-solo client already receives. */
	}
	return ret;
}
		...
		const bool stale)
{
	 ...
	 * included. generate_coinbase() emits nSequence 0xffffffff with
	 * nLockTime 0, which is final unconditionally, so this cannot fire
	 * today -- it is here because the combination that breaks it is one
	 * careless edit away and every upstream rebase re-introduces upstream's
	 * BIP54 nSequence/nLockTime pair, whose half-applied revert this
	 * catches.
	 *
	 * Deliberately DIAGNOSTIC ONLY: it reports and still submits. A block
	 * that really is non-final is lost either way, and submitting it buys
	 * bitcoind's authoritative "bad-txns-nonfinal" as evidence -- whereas a
	 * false positive in this check, if it could refuse, would throw away a
	 * perfectly good block. This fork has already shipped one guard that
	 * silently refused to submit a solved block (the 511-byte hexcoinbase
	 * bound removed by upstream 2baff8f7); do not make that trade twice. */
	{
		uint32_t cb_seq, cb_locktime;
		coinbase_final_t fin = coinbase_finality((const unsigned char *)coinbase,
							 cblen, wb->height, &cb_seq,
							 &cb_locktime);

		if (unlikely(fin != CBF_FINAL)) {
			LOGEMERG("SUBMITTING BLOCK WITH %s COINBASE at height %d: nSequence %08x nLockTime %u (%d bytes). "
				 "Consensus requires a final coinbase; expect bad-txns-nonfinal.",
				 fin == CBF_NONFINAL ? "NON-FINAL" :
				 fin == CBF_UNKNOWN ? "POSSIBLY NON-FINAL" : "UNPARSEABLE",
				 wb->height, cb_seq, cb_locktime, cblen);
		}
	}
...
/* Entered with instance_lock held. Returns the coinb2 this client was
 * actually given for this workbase, or NULL if the pool does not have one.
 *
 * In solo mode every authorised client has a userwb generated for it
 * (address users, and pool-fallback users per decision D2) before it can
 * receive work, and clear_userwb() frees a userwb only when its workbase is
 * freed -- so a missing userwb for a live workbase should not happen, and
 * has not: `missing userwb` occurs 0 times in 77,067 lines of production log.
 *
 * NULL is nevertheless the only safe answer, because there is no coinbase
 * here that can be guessed:
 *
 *  - wb->coinb2bin alone is malformed in solo mode. Its output-count byte
 *    says 1 or 2 but it carries no output scripts at all.
 *  - Splicing the POOL's own output script (sdata->txnbin) in, which this
 *    function used to do, produces a well-formed coinbase that is still the
 *    wrong one: mining.notify sent this client userwb->coinb2 carrying the
 *    MINER's script, so a different coinbase means a different merkle root
 *    and every share validates to noise. Worse, test_blocksolve() runs on
 *    whatever coinbase was assembled here -- so a genuine solve on the
 *    miner's coinbase would be missed, and a solve found on the spliced one
 *    would be SUBMITTED AS A BLOCK PAYING ckpool.btcaddress INSTEAD OF THE
 *    MINER. On a solo pool the block reward is the entire product.
 *
 * The caller rejects the share with SE_NO_USER_COINBASE and re-notifies the
 * client with work it can actually mine. */
static inline uchar *__user_coinb2(const stratum_instance_t *client,
				   const workbase_t *wb, int *cb2len)
{
	struct userwb *userwb;
	int64_t id;
 ...
 *
 * Returns the share difficulty, or SUBMISSION_NO_COINBASE if this client has
 * no per-user coinbase for this workbase -- see __user_coinb2(). In that case
 * nothing is hashed, hash is zeroed, and the caller must reject the share:
 * there is no coinbase to validate against and guessing one risks paying a
 * solved block to the wrong address. */
			      ...
			      const bool stale)
{
	...
	if (unlikely(!coinb2bin)) {
		ck_runlock(&sdata->instance_lock);
		memset(hash, 0, 32);
		return SUBMISSION_NO_COINBASE;
	}
...
bool stratifier_sv2_snapshot_work(struct sv2_work_snap *out, int64_t instance_id)
{
	...
	if (client) {
		...
		if (ckpool.btcsolo && client->user_instance &&
			...
			__generate_userwb(sdata, wb, client->user_instance);
		coinb2bin = __user_coinb2(client, wb, &cb2len);
		/* NULL means no per-user coinbase exists for this workbase.
		 * Leave ok false rather than sending the client work the pool
		 * cannot validate a share against. */
		if (coinb2bin && cb2len <= (int)sizeof(out->coinb2)) {
				 ...
				 char *errbuf, size_t errbufsz, double *sdiff_out)
{
	...
	sdiff = submission_diff(sdata, client, wb, nonce2hex, ntime, version, true,
				noncehex, hash, stale);
	if (unlikely(sdiff == SUBMISSION_NO_COINBASE)) {
		/* Same reasoning as the SV1 path in parse_submit(): there is
		 * no coinbase to validate against and none may be guessed. */
		snprintf(errbuf, errbufsz, "invalid-job-id");
		LOGWARNING("SV2 instance %"PRId64" user %s has no coinbase for solo workbase %"PRId64
			   ", rejecting share", instance_id,
			   client->user_instance->username, workbase_id);
		renotify_solo_client(sdata, client);
		put_workbase(sdata, wb);
		dec_instance_ref(sdata, client);
		return false;
	}
			 ...
			 enum share_err *err_code)
{
	...
	/* The submitted version bits, rendered for the sharelog. Always 8 hex
	 * digits; "00000000" when the client sent no params[5]. */
	char version_mask_hex[9];
	user_instance_t *user = client->user_instance;
	...
	if (version_mask && strlen(version_mask) && validhex(version_mask)) {
		...
		if (version_mask32 && ((~ckpool.version_mask) & version_mask32) != 0) {
			...
			/* NOTICE, not INFO: the goto below leaves parse_submit
			 * before the sharelog record is built, so this line is
			 * the only place the rejection is ever recorded.
			 * Prints the parsed values only -- the raw params[5]
			 * string is client controlled and unsanitised. */
			LOGNOTICE("Rejected client %s out of mask version bits: sent %08x, pool mask %08x, disallowed %08x",
				  client->identity, version_mask32, ckpool.version_mask,
				  version_mask32 & ~ckpool.version_mask);
			goto out;
		}
	}
	...
	sdiff = submission_diff(sdata, client, wb, nonce2, ntime32, version_mask32, false,
				nonce, hash, stale);
	if (unlikely(sdiff == SUBMISSION_NO_COINBASE)) {
		/* No coinbase for this client on this workbase, so the share
		 * cannot be validated against the work the client was given
		 * and MUST NOT be validated against a guessed one -- see
		 * __user_coinb2(). Reject it and hand the client fresh work
		 * off the current workbase, generating its userwb if that is
		 * what was missing. hexhash stays empty in the sharelog on
		 * purpose: no header was built, so there is no hash to report.
		 */
		err = SE_NO_USER_COINBASE;
		LOGWARNING("Client %s user %s has no coinbase for solo workbase %"PRId64
			   ", rejecting share and re-notifying",
			   client->identity, user->username, id);
		renotify_solo_client(sdata, client);
		goto out_put;
	}
	if (sdiff > client->best_diff) {
...
out_nowb:
	 ...
	 * record already is. Emitted as the PARSED value rather than the raw
	 * string, because that is the number ORed into the header the pool
	 * hashed -- it normalises a client that pads, uppercases or 0x-prefixes
	 * its mask, which is exactly the disagreement this field exists to
	 * expose. Two independent investigations into ~2,200 rejected shares
	 * were unable to prove the mechanism for one incident purely because
	 * this was the one submitted field ckpool discarded after use. */
	snprintf(version_mask_hex, sizeof(version_mask_hex), "%08x", version_mask32);

	doc = yyjson_mut_pack("{sIsIsssssssssfsfsssbsssissssssssssssssssss}",
		"workinfoid", id,
		...
		/* Appended last on purpose: a consumer reading this record by key
		 * is unaffected, and one reading positionally keeps every
		 * existing index. */
		"version_mask", version_mask_hex);
...
static user_instance_t *generate_remote_user(const char *workername)
{
	...
	/* Same identity split as generate_user(): a bare-spelled remote miner
	 * must not land on a phantom account distinct from the prefixed one. */
	canonicalise_username(username, canon_username, sizeof(canon_username));
	username = canon_username;
	...
	if (!ckpool.proxy && (new_user || !user->btcaddress)) {
		 ...
		 * stores it, so a throwaway local receives it. */
		bool segwit_unused;
...
static void parse_remote_share(sdata_t *sdata, yyjson_mut_val *val, const char *buf)
{
	...
	worker->shares += diff;
	user->shares += diff;
	/* A remote user IS the same key as a local user of that name (that is
	 * what generate_remote_user() already does), so their round accrues
	 * identically to the local share path. */
	user->round_shares += diff;
	tv_time(&now_t);
 ...
 * explicit solo-mode address-typo error set in parse_authorise()'s err_doc)
			       ...
			       const char *buf, const int64_t client_id)
{
	...
	const char *finder_workername = yyjson_mut_get_str(yyjson_mut_obj_get(val, "workername"));
	user_instance_t *finder = NULL;
	yyjson_mut_doc *res;
	workbase_t *wb = NULL;
	double diff = 0;
	...
	/* Untrusted remote input: find-only lookup, never creates a user.
	 * Strip the worker suffix the same way user_by_workername() does,
	 * since users are hashed by bare username; workername "" (or a
	 * missing/non-string value) simply misses and leaves finder NULL.
	 * Canonicalise too, or a bare-spelled remote finder misses the
	 * prefixed account it actually belongs to. */
	if (finder_workername) {
		char *finder_username = strdupa(finder_workername), *ignore = finder_username;
		char canon_finder[CANON_USERNAME_SIZE];

		strsep(&ignore, "._");
		canonicalise_username(finder_username, canon_finder, sizeof(canon_finder));
		finder = find_user(sdata, canon_finder);
	}
	...
	if (unlikely(!wb))
		...
		if (local_block_submit(gbt_block, flip32, wb->height)) {
			block_share_summary(sdata);
			/* finder is NULL here when the remote block names a
			 * user find_user() doesn't know -- an unattributable
			 * solve, not a reset-everyone one. */
			if (finder)
				reset_bestshares(sdata, finder);
			else
				reset_bestshares_unattributed(sdata);
...
static void sauth_process(json_params_t *jp)
{
	...
	if (ret) {
		...
		const char *reason = err_doc ?
			yyjson_mut_get_str(yyjson_mut_doc_get_root(err_doc)) : NULL;
	...
	/* Send the difficulty immediately after the authorisation response
	 * regardless of whether it changed here. NiceHash and MiningRigRentals
	 * expect mining.set_difficulty right after mining.authorize; without it
	 * a rental rig mines the startdiff until the first vardiff adjustment
	 * and the marketplace flags the pool. parse_authorise() has already
	 * clamped whatever it assigned into [mindiff, maxdiff], so this cannot
	 * announce an out-of-range difficulty. */
	if (client->diff > 0) {
		stratum_send_diff(sdata, client);
		LOGINFO("Sent initial difficulty %"PRId64" to client %s after authorization",
...
static void *statsupdate(void __maybe_unused *arg)
{
	...
	while (42) {
		...
		while ((user = next_user(sdata, user)) != NULL) {
			...
			/* Per-user round luck: this user's accumulated share
			 * difficulty since the last block, as a percentage of
			 * network difficulty. round_shares is zeroed by
			 * reset_bestshares() for the finder on an attributed
			 * block solve, or for every user on an operator reset,
			 * and left untouched for everyone when a solve can't be
			 * attributed (reset_bestshares_unattributed()), so this
			 * is round progress, directly comparable to the
			 * pool-wide "luck" in pool.status. 100% means the user
			 * has done a full network difficulty of work without
			 * solving -- i.e. running unlucky. networkdiff is
			 * emitted alongside it so consumers can recompute it.
			 * A zero network_diff would be a fatal divide by zero
			 * as ckpool unmasks FE_DIVBYZERO. */
			user_luck = 0;
			if (likely(stats->network_diff > 0))
				user_luck = (double)user->round_shares * 100 / stats->network_diff;

			doc = yyjson_mut_pack("{ss,ss,ss,ss,ss,sI,si,sI,sI,sf,sf,sf,sI,sI}",
				"hashrate1m", suffix1,
				"hashrate5m", suffix5,
				"hashrate1hr", suffix60,
				"hashrate1d", suffix1440,
				...
				"shares", user->shares,
				"roundshares", user->round_shares,
				"bestshare", user->best_diff,
				"luck", user_luck,
				"networkdiff", stats->network_diff,
		...
		if (likely(stats->network_diff > 0))
			percent = round((double)stats->accounted_diff_shares * 10000 /
					stats->network_diff) / 100;
		else
			percent = 0;
		/* This value is the accumulated share difficulty as a
		 * percentage of network difficulty -- i.e. round progress,
		 * which is mining LUCK, not a difficulty. It is emitted twice
		 * on purpose: "luck" is the honest name, "diff" is the upstream
		 * ckpool misnomer kept as a deprecated alias.
		 *
		 * "diff" MUST remain the FIRST key on this line: consumers
		 * (BlockSniper Laravel, PoolDataCollectionService) locate it by
		 * grepping the literal prefix 'Pool:{"diff"'. Append new fields
		 * after it, never before.
		 *
		 * FP_TO_FIXED(2), not upstream's (1): percent is rounded to two
		 * decimal places just above, and one decimal place of output
		 * would throw the second away. */
		doc = yyjson_mut_pack("{sf,sf,sI,sI,sI,sf,sf,sf,sf}",
		        "diff", percent,
		        "luck", percent,
...
static void *zmqnotify(void __maybe_unused *arg)
{
	...
	/* The fork subscribes to every btcd's own "zmqnotify" endpoint rather
	 * than the single global zmqblock, so a two-node production topology
	 * learns about a new block from whichever node sees it first. Falls
	 * back to the single zmqblock endpoint when no per-btcd endpoint is
	 * configured. */
	if (ckpool.btcdzmq_count > 0) {
		num_endpoints = ckpool.btcdzmq_count;
		LOGNOTICE("Connecting to %d ZMQ endpoints from btcd config", num_endpoints);
	} else if (ckpool.zmqblock) {
		num_endpoints = 1;
		LOGNOTICE("Using single ZMQ endpoint: %s", ckpool.zmqblock);
	} else {
	...
	while (42) {
		...
		for (i = 0; i < num_endpoints; i++) {
			...
			if (!subscribers[i] || !(poll_items[i].revents & ZMQ_POLLIN))
				continue;
			...
			while (more) {
				...
				/* Read the multipart flag BEFORE closing: the
				 * upstream loop tests zmq_msg_more() on a
				 * already-closed message. */
				more = zmq_msg_more(&message);
				zmq_msg_close(&message);
			}
			LOGDEBUG("ZMQ message complete from endpoint %d", i);
		}
	}
...
void *stratifier(void *arg)
{
	...
	if (!ckpool.proxy) {
		/* segwit is always false on BCH; ckpool no longer stores it, so
		 * throwaway locals receive the output params. */
		bool segwit_unused, poolsegwit_unused;

		if (!generator_checkaddr(ckpool.btcaddress, &ckpool.script, &segwit_unused)) {
		 ...
		 * BCH classifier cannot encode at all, so take it from the
		 * CashAddr prefix the generator detected off getblockchaininfo
		 * instead. It gates BIP34 height serialisation in
		 * generate_coinbase(). */
		ckpool.regtest = !safecmp(bch_get_cashaddr_prefix(), "bchreg");

		/* Validate the pool operator fee address */
		if (ckpool.pooladdress && ckpool.poolfee > 0) {
			if (generator_checkaddr(ckpool.pooladdress, &ckpool.poolscript, &poolsegwit_unused)) {
				...
				LOGWARNING("Pool fee address %s is invalid, disabling pool fee", ckpool.pooladdress);
				ckpool.poolfee = 0;
			}
		}
	}
