...
CashStratum implements a dual-output coinbase mechanism that automatically splits block rewards between miners and the pool operator. This document explains the mechanics, configuration, and verification.
...
CashStratum includes an end-to-end test suite at `testing/regtest-e2e.sh` that:
...
*CashStratum Pool Fee System — Non-custodial, dual-output, solo mining with automatic operator fee distribution*
