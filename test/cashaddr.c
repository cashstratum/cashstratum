...
int main(void)
{
	...
	/* ---- invalid: non-ASCII bytes (out-of-bounds read) ----
	 *
	 * CHARSET_REV has 128 entries and tolower() returns bytes >= 0x80
	 * unchanged in the C locale, so before the guard these indexed up to
	 * 128 bytes past the end of the table. Reachable pre-auth: the string
	 * being decoded is the miner-supplied username from mining.authorize.
	 *
	 * These assert rejection, which is all that is observable from here --
	 * the out-of-bounds READ itself is only caught by a sanitiser, so run
	 * this binary under ASAN/UBSAN to see the original failure. Every byte
	 * from 0x80 to 0xff is swept because the table values past the end are
	 * whatever the linker put there, and a positive value at the wrong
	 * offset would make the decoder ACCEPT a byte that is not in the
	 * charset at all. */
	{
		unsigned b;
		...
		for (b = 0x80; b <= 0xff; ++b) {
			...
			snprintf(desc, sizeof(desc), "non-ASCII byte %02x rejected without reading past CHARSET_REV", b);
			check(!cashaddr_decode_checked(buf, "bitcoincash", got_hash160, &is_p2sh), desc);
		}
		/* Also with the prefix present, which takes the other branch
		 * through the separator handling. */
		snprintf(buf2, sizeof(buf2), "bitcoincash:%s", strchr(VALID_VECTORS[0].cashaddr, ':') + 1);
		buf2[12] = (char)0xff;
		check(!cashaddr_decode_checked(buf2, "bitcoincash", got_hash160, &is_p2sh),
		      "non-ASCII byte in a prefixed address rejected");
	}
