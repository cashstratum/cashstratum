...
CKPOOL_CONF="$INSTALL_DIR/cashstratum.conf"
...
if [ -f /etc/systemd/system/bitcoind.service ] || [ -f /etc/systemd/system/cashstratum.service ] || [ -f /etc/systemd/system/ckpool.service ] || [ -d /opt/cashstratum ] || [ -d /opt/ckpool ] || [ -d /etc/cashstratum ] || [ -d /etc/ckpool ] || [ -f /usr/local/bin/wait-for-bitcoind-sync.sh ]; then
...
existing_donation=""
existing_btcsig=""
if [ ! -f "$CKPOOL_CONF" ] && [ -f "/etc/ckpool/ckpool.conf" ]; then
...
if [ -n "$existing_btcsig" ]; then
    ...
    read -p "Enter an optional signature string to include in the coinbase of mined blocks (leave blank for 'CashStratum'): " btcsig
    if [ -z "$btcsig" ]; then
        btcsig="CashStratum"
    fi
fi
...
STAGE_DIR=$(mktemp -d /opt/.cashstratum-build-XXXXXX)
git clone https://github.com/cashstratum/cashstratum.git "$STAGE_DIR/cashstratum"
(
    cd "$STAGE_DIR/cashstratum"
    ./autogen.sh
    ./configure --prefix=/opt/cashstratum/build --disable-sv2
    make -j"$(nproc)"
)

if [ ! -f "$STAGE_DIR/cashstratum/src/ckpool" ]; then
    echo "Build failed!"
    exit 1
fi
...
systemctl stop cashstratum 2>/dev/null || true
...
# Fix Trap T9: do not blindly rm -rf /opt/ckpool; install cleanly to /opt/cashstratum
INSTALL_DIR="/opt/cashstratum"
mkdir -p "$INSTALL_DIR"/{logs,users,pool,data,logs/shares}

# Copy CashStratum binaries
cp "$STAGE_DIR/cashstratum/src/ckpool" "$INSTALL_DIR/cashstratum"
ln -sf cashstratum "$INSTALL_DIR/ckpool"
cp "$STAGE_DIR/cashstratum/src/ckpmsg" "$INSTALL_DIR/"
cp "$STAGE_DIR/cashstratum/src/notifier" "$INSTALL_DIR/"
[ -f "$STAGE_DIR/cashstratum/clean-old-blocks.sh" ] && cp "$STAGE_DIR/cashstratum/clean-old-blocks.sh" "$INSTALL_DIR/"
chmod +x "$INSTALL_DIR"/{cashstratum,ckpool,ckpmsg,notifier} 2>/dev/null || true
[ -f "$INSTALL_DIR/clean-old-blocks.sh" ] && chmod +x "$INSTALL_DIR/clean-old-blocks.sh" 2>/dev/null || true

# Copy Go API source if present
if [ -d "$STAGE_DIR/cashstratum/api" ]; then
    mkdir -p "$INSTALL_DIR/api"
    cp -r "$STAGE_DIR/cashstratum/api"/* "$INSTALL_DIR/api/"
fi

# Cleanly symlink /opt/ckpool -> /opt/cashstratum
if [ -L /opt/ckpool ]; then
    rm -f /opt/ckpool
elif [ -d /opt/ckpool ] && [ "/opt/ckpool" != "$INSTALL_DIR" ]; then
    rm -rf /opt/ckpool
fi
ln -sfn /opt/cashstratum /opt/ckpool

# Global CLI symlinks
ln -sf "$INSTALL_DIR/cashstratum" /usr/local/bin/cashstratum
ln -sf "$INSTALL_DIR/cashstratum" /usr/local/bin/ckpool
...
cat << EOF > "$CKPOOL_CONF"
{
  ...
  "sockdir" : "/tmp/cashstratum",
  "logdir" : "$INSTALL_DIR/logs",
  "startdiff" : 10000
}
EOF
chmod 640 "$CKPOOL_CONF"
ln -sf cashstratum.conf "$INSTALL_DIR/ckpool.conf"

mkdir -p /etc/cashstratum /etc/ckpool
ln -sf "$CKPOOL_CONF" /etc/cashstratum/cashstratum.conf
ln -sf "$CKPOOL_CONF" /etc/ckpool/ckpool.conf
chown -R $service_user:$service_user "$INSTALL_DIR" /etc/cashstratum /etc/ckpool
...
CKLOG=/opt/cashstratum/logs/cashstratum.log
[ -f "$CKLOG" ] || CKLOG=/var/log/ckpool/ckpool.log
...
ln -sf /usr/local/bin/ckpool-mining-urls.sh /usr/local/bin/cashstratum-mining-urls.sh
...
cat << EOF > /etc/systemd/system/cashstratum.service
[Unit]
Description=CashStratum Solo Mining Pool
...
ExecStart=/opt/cashstratum/cashstratum -c /opt/cashstratum/cashstratum.conf -n cashstratum -L
...
# Setup Go log API if Go is installed
if command -v go >/dev/null 2>&1 && [ -d "$INSTALL_DIR/api" ]; then
    echo "Setting up Go log API..."
    API_DIR="$INSTALL_DIR/api"
    if (cd "$API_DIR" && go build -ldflags="-s -w" -o "$API_DIR/cashstratum-api" .); then
        chmod +x "$API_DIR/cashstratum-api"
        ln -sf cashstratum-api "$API_DIR/ckpool-api"
...
CKPOOL_LOG_PATH=$INSTALL_DIR/logs/cashstratum.log
...
ENVEOF
            ...
            ln -sf cashstratum-api.env "$API_DIR/ckpool-api.env"
        fi
        chown -R $service_user:$service_user "$API_DIR"

        cat << EOF > /etc/systemd/system/cashstratum-api.service
[Unit]
...
ExecStart=$API_DIR/cashstratum-api
EnvironmentFile=$API_DIR/cashstratum-api.env
Restart=always
RestartSec=5
MemoryMax=128M
CPUQuota=25%
...
EOF
        ...
        systemctl enable cashstratum-api
        systemctl start cashstratum-api 2>/dev/null || true
    fi
fi

systemctl daemon-reload
systemctl enable bitcoind cashstratum
systemctl start bitcoind cashstratum
...
echo "  - journalctl -u cashstratum -f (block progress until CKPool starts)"
echo "  - journalctl -u bitcoind -f (detailed sync logs)"
echo "  - tail -f $DATADIR/debug.log (detailed sync logs)"
echo "CKPool startup is delayed until sync completes (monitor with: journalctl -u cashstratum -f)."
echo
...
echo "Monitor logs:"
echo "  - CKPool: tail -f /var/log/ckpool/ckpool.log (full logs) or journalctl -u cashstratum -f (block progress, then reduced CKPool logs)"
echo "  - Bitcoin Core: tail -f $DATADIR/debug.log or journalctl -u bitcoind -f"
echo "Edit configs in $BITCOIN_CONF and $CKPOOL_CONF if needed, then restart services with: systemctl restart bitcoind cashstratum"
