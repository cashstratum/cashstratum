...
[![Mainnet Blocks](https://img.shields.io/badge/MAINNET%20BLOCKS-69%2B-0281F5?style=for-the-badge&labelColor=030711)](#-production-achievements)
...
This is the **private working repository**. It is the source that the public mirror
(`skaisser/ckpool`) is synced from, and it is what production is built from.

| Environment | Host | Stratum | Coinbase tag |
|---|---|---|---|
| **Production** | `solo.blocksniper.ai` (45.63.84.63) | `stratum+tcp://solo.blocksniper.ai:3333` | `/BlockSniper.ai/ AI Smart Solo Mining` |
| **Testing / build** | `beast` (homelab, 10.0.0.5) | internal only | `/BlockSniper.ai/ AI Smart Solo Mining` — realigned to production |

**Miners authenticate with a Bitcoin Cash address as the stratum username** — CashAddr with the
`bitcoincash:` prefix, CashAddr bare, UPPERCASE CashAddr, or legacy Base58 all resolve to the
same account, and the block reward is paid **directly to that address on-chain** in the coinbase
of the block it finds, minus the operator fee.

```bash
./cgminer -o stratum+tcp://solo.blocksniper.ai:3333 \
          -u bitcoincash:qz280jyp563e99fjsvua4gpt6nu74n7engf60yj4gk -p d=500000
```

Verified live against production: the coinbase carries the operator's `btcsig`, output 0 pays the
exact address supplied as the username, and output 1 takes the operator fee at 2.00%. Blocks
mined before 2026-09-04 carry the previous tag `BlockSniper.ai/[Solo]`; earlier historical blocks
carry `EloPool.cloud/[Solo]`. All three are the same software.

Every miner gets a public stats page keyed by their payout address — hashrate, workers, best
share and every block they have found:

```
https://blocksniper.ai/miner/<your-bch-address>
```
...
BlockSniper.ai is a **heavily modified BCH-only fork** that fixes all four, and has **69 blocks
...
**Live and healthy on BCH mainnet.** Public per-miner stats page:
[`blocksniper.ai/miner/qzxq7lc575az8tw357ck50wdl7cmn4lc2v9kp3rz4u`](https://blocksniper.ai/miner/qzxq7lc575az8tw357ck50wdl7cmn4lc2v9kp3rz4u)

- **69 blocks mined on BCH mainnet** — proven reliability in production
- **Most recent: block 966569** (2026-09-01), solved by a NiceHash-rented worker at
  **443 TH/s (1 m)** against a network difficulty of 4.39 × 10¹¹ — the full solve, finder
  identity and hashrate are recoverable from the log via
  [`GET /find-block?height=`](#block-results-and-finder-hashrate-by-height)
- **Sustained multi-petahash operation** — the pool has carried **100 PH/s+** of rented
  hashrate through 1 m/5 m/1 h/1 d windows without a stratum stall
...
- **Native CashAddr working** — proven with billions of shares
- **Pool fees working** — automatic 98/2% split in every block
- **Dual-node active-active** — sub-100 ms failover, both BCH nodes fully synced
...
> ckpool's own 38-byte cap bites first. Sizing a signature against 100 will get it truncated.
...
```
!K j W 3 5g j5~fmN  /BlockSniper.ai/ AI Smart Solo Mining
```

That prefix is protocol, not a misconfiguration, and it cannot be removed. Every ckpool block
looks like this.
...
> **This is how BlockSniper runs in production right now** — two active-active nodes, both
> ZMQ feeds, first notification wins. Live topology, IPs and the deployed conf shape (no
> credentials) are documented in [PRODUCTION.md](PRODUCTION.md).
...
- **testnet4 block submission is NOT proven, and this is not a defect.** BCH testnet has a
  ...
  2. **`update_interval` decides whether you are even present.** ckpool refreshes its block
     ...
     *block*, not for a template whose difficulty dropped under an unchanged tip. In one observed
     window ckpool logged `Network diff set to 1.0` **two seconds after** the winning block was
...
- **mainnet has solved a real block with this software** — 966569, on 2026-09-01: two outputs,
  ...
  One honest caveat: that block predates the current coinbase signature, so it carries the older
  `BlockSniper.ai/[Solo]` tag. The signature in use now has been proven byte-exact in a *served
  template* on mainnet and in an *accepted block* on regtest, but has not yet appeared in a
  mainnet block — no block has been found since it changed. Nothing about the signature affects
  block validity (it is the last field of the coinbase scriptSig, well inside the 100-byte cap),
  so this is a matter of waiting for a block, not an open risk.
...
| **Killing a dependency mid-run** | Node outage / failover | Start the e2e, `bitcoin-cli stop` its regtest node partway through, and stream ckpool's own logfile out before the harness deletes its workdir. Log **cadence** is the signal — a thread that stops logging on schedule is a thread that is blocked. |

> [!IMPORTANT]
> **ckpool writes to its own logfile, not to the harness's stdout.** Grepping the e2e's output for
> a ckpool log line returns zero matches whether or not the line fired, which reads exactly like a
...
> until LOGF=$(ls /tmp/keep/ckpool-e2e.*/logs/e2e.log 2>/dev/null); do sleep 1; done
> tail -n +1 -F "$LOGF" > /tmp/ckpool-captured.log &
> ```
...
Both stats objects carry the same five windows, formatted by ckpool's `suffix_string()` as a
...
> **Fetch promptly and persist.** `/find-block` greps the **live** `ckpool.log`; it is not a
> block database. ckpool ships no log rotation, so how far back a height stays resolvable is
...
ckpool confirms a solve — that event is the trigger, `/find-block` is the detail fetch.
