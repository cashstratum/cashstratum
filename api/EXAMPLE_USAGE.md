...
Every response below was captured live from the reference production deployment (build `464a7ac6`,
...
Miner IP addresses in the captures are replaced with RFC 5737 documentation addresses.

## Basics

- Base URL: `http://<pool-host>:8888` — never exposed to the internet or browsers; Laravel
  proxies everything.
- Every endpoint needs `Authorization: Bearer $CASHSTRATUM_API_KEY` (with `$CKPOOL_API_KEY`
  supported for backwards compatibility). Missing/wrong key → `401` with plain-text body `Unauthorized`.
...
```bash
curl -H "Authorization: Bearer $CASHSTRATUM_API_KEY" "http://$POOL:8888/<endpoint>"
```
...
- `hashrate` is CashStratum's SI-suffixed string (`"50.8P"`, `"2.31G"`, `"0"`) — parse suffix-aware,
  never as a float.
- `block_height` and `node` come from the BCH node's own `getblockchaininfo`, using the RPC
  credentials already in `cashstratum.conf` (or legacy `ckpool.conf`). The same capture from the host's `bch-status` helper
...
- `node.difficulty` is the current network difficulty. This value should match `networkdiff` in
  each miner's user file (via `/user-file?user=…`). A persistent divergence means CashStratum is
...
since build `464a7ac6` it also carries `"luck"` (same value — `diff` is the deprecated alias
...
- **`worker` is the field to render.** CashStratum stores the whole stratum auth string in
...
- `luck` / `roundshares` / `networkdiff` (new in build `464a7ac6`): the user's **independent**
  ...
  authorised since a CashStratum restart — the file may be frozen from an older build. Consumers
  ...
  - **Note:** `networkdiff` (in the user file) should match `/stats.node.difficulty`; a
    persistent divergence means CashStratum is working on a stale template and needs a restart.
...
- The middle is the `.sharelog` file inside it. **Empty is normal and valid**: it means "the
  start of this height dir", which is what an idle pool produces — CashStratum has created the
...
- `user` filter resolves both CashAddr spellings (bare `qz…` and `bitcoincash:qz…` forms),
  ...
  spellings are treated as equivalent, matching CashStratum's own accounting.
