...
if [ "$EUID" -eq 0 ]; then
   echo -e "${RED}Please donx27t run this script as root${NC}"
   exit 1
fi
...
cat > "$INSTALL_DIR/cashstratum-testnet.conf" << CONF_TESTNET_EOF
{
    "btcd": [{
        "url": "127.0.0.1:18332",
        "auth": "yourrpcuser",
    ...
    "logdir": "logs",
    "sockdir": "/tmp/cashstratum-testnet",
    "node_warning": false,
    "log_shares": true,
    "asicboost": true,
    "version_mask": "1fffe000",
...
CONF_TESTNET_EOF
ln -sf cashstratum-testnet.conf "$INSTALL_DIR/ckpool-testnet.conf"
...
pkill -TERM cashstratum || pkill -TERM ckpool
sleep 2

if pgrep -x "cashstratum" > /dev/null || pgrep -x "ckpool" > /dev/null; then
    echo "Force stopping..."
    pkill -9 -x cashstratum 2>/dev/null || true
    pkill -9 -x ckpool 2>/dev/null || true
fi
...
ln -sf stop-cashstratum.sh "$INSTALL_DIR/stop-ckpool.sh"
...
# ---------------------------------------------------------------------------
API_PORT="${CASHSTRATUM_API_PORT:-${CKPOOL_API_PORT:-8888}}"
...
if ! command -v go &> /dev/null; then
    ...
    if (cd "$CURRENT_DIR/api" && go build -ldflags="-s -w" -o "$API_DIR/cashstratum-api" .); then
        ...
        if [ -f "$API_DIR/cashstratum-api.env" ]; then
            ...
            echo -e "${YELLOW}  Existing ckpool-api.env found - preserving existing API key${NC}"
            OLD_KEY=$(sed -n "s/^CKPOOL_API_KEY=//p" "$API_DIR/ckpool-api.env" | tail -1)
            [ -z "$OLD_KEY" ] && OLD_KEY=$(openssl rand -hex 32)
            (
umask 077
cat > "$API_DIR/cashstratum-api.env" <<ENVEOF
# Generated by install-cashstratum.sh. Keep this file at mode 0600.
CASHSTRATUM_API_KEY=$OLD_KEY
CKPOOL_API_KEY=$OLD_KEY
...
echo "  • cashstratum-testnet.conf - Testnet configuration (port 3334)"
echo
...
echo "  ./start-cashstratum.sh cashstratum-testnet.conf     # Uses testnet config"
echo
echo "To monitor:"
